...
class SiteStageRevisionTests(TestCase):
    ...
    def test_fabrica_stage_revision(self):
        # Cria um site no escopo FABRICA
        site_fabrica = Site.objects.create(
            site_id='FABRICA_REV_01',
            name='Site Fábrica Revisavel',
            scope_type='FABRICA',
            planned_survey_date='2026-07-02',
            planned_report_date='2026-07-10'
        )
        # Sincroniza stages para colocar a etapa Projeto como DONE
        proj_stage = site_fabrica.stages.get(stage_name='Projeto')
        proj_stage.status = 'DONE'
        proj_stage.save()

        self.client.login(username='revision_user', password='password123')
        url = reverse('site_detail', kwargs={'pk': site_fabrica.pk})
        ...
        response = self.client.post(url, {
            ...
            'stage_name': 'Projeto',
            'request_date': '2026-07-12',
            'reason': 'Ajustar layout da fábrica no projeto.'
        })
        self.assertEqual(response.status_code, 302)
